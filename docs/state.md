...
- Backend Lambda resource provisioning
  ...
  - Use an HTTP API + Cognito JWT authorizer to perform signature verification and expiration handling
...
- Consideration of the error page
- Linter and formatter integration
  - ESLint (Flat Config) with type-aware rules
    - Vue + TypeScript across `apps/frontend`, `apps/backend`, and `infra/cdk`
    - Plugins: `typescript-eslint`, `eslint-plugin-vue`, `eslint-plugin-import-x`, `eslint-plugin-unused-imports`
  - Prettier as the single formatter (`eslint-config-prettier` disables conflicting rules)
  - `pnpm lint` / `pnpm lint:fix` / `pnpm format` / `pnpm format:check` available at the repo root and in each workspace
